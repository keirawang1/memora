...
/**
 * Free batched refresh — catalog embeddings + template reasons only (no Gemini).
 * Each cited pick is same media type as the seed and nearest to that seed's embedding.
 */
...
async function generateForUser(
  ...
  const { data: cached } = await admin
    ...
    .select('expires_at, input_fingerprint')
    .eq('user_id', userId)
    .maybeSingle();
  if (
    cached &&
    ...
    const cachedItems = await admin
      .from('user_recommendations')
      .select('items')
      .eq('user_id', userId)
      .maybeSingle();
    const len = Array.isArray(cachedItems?.items) ? (cachedItems.items as unknown[]).length : 0;
    if (len > 0) return 0;
  }
  ...
  const { data: allTitles } = await admin.from('media').select('title').eq('user_id', userId);
  ...
  const { data: prev } = await admin
    ...
    .eq('user_id', userId)
    .maybeSingle();
  const previousIds = ((prev?.items as { catalog_id?: string }[]) ?? [])
    .map((i) => i.catalog_id)
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const { data: rated } = await admin
      ...
      .gt('rating', HIGH_RATING)
      .limit(200);
    for (const r of rated ?? []) ids.add(String(r.user_id));
    ...
    const { data: expired } = await admin
      ...
      .limit(50);
    for (const r of expired ?? []) ids.add(String(r.user_id));
    ...
    const { data: job } = await admin
      ...
      .eq('id', 'refresh-recs')
      .maybeSingle();
    const offset = Number((job?.cursor as { offset?: number } | null)?.offset ?? 0);
    const all = [...ids];
    const batch = all.slice(offset, offset + USERS_PER_RUN);
    const nextOffset = offset + USERS_PER_RUN >= all.length ? 0 : offset + USERS_PER_RUN;

    let ok = 0;
    const errors: string[] = [];
    for (const userId of batch) {
      try {
        await generateForUser(admin, userId);
        ok += 1;
      } catch (err) {
    ...
    await admin.from('recommendation_jobs').upsert({
      id: 'refresh-recs',
      status: 'idle',
      cursor: { offset: nextOffset, pool: all.length },
      finished_at: new Date().toISOString(),
      last_error: errors[0] ?? null,
      updated_at: new Date().toISOString(),
    });

    return new Response(JSON.stringify({ processed: batch.length, ok, nextOffset, errors }), {
      headers: { ...corsHeaders, 'Content-Type': 'application/json' },
    });
  } catch (err) {
    return new Response(
